...
% Make longtable captions span full text width
...
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{float} % For strict figure placement with [H]
...
Language models offer efficiency to users but are highly susceptible to hallucinations, 
leading to significant concerns in security and privacy. This research evaluates eight 
mitigation strategies on small language models using the TruthfulQA benchmark. The 
experiments measure factual accuracy and semantic consistency, aiming to find out 
the most efficient hallucination prevention strategies. It is found that contrastive 
decoding is the most effective defense through an improvement in all evaluation metrics. 
Alongside performance comparison across methods, the results demonstrate the effectiveness 
of different strategies on enhancing the trustworthiness of resource-constrained models. 

\section{Introduction}

Artificial intelligence (AI) is reshaping industries and schools
across the world through various applications, such as
...
(Jones, 2025; IEEE Xplore, 2024). AI hallucinations can lead to
significant security risks by causing navigation errors, financial losses, 
incorrect medical diagnoses, and missed security threats(Sood et al., 2025). 
This research paper aims to provide an overview of the effectiveness of 
different strategies in reducing hallucination rate.
...
despite their rapid adoption across diverse domains, their limitations remain 
largely overlooked. Among all challenges, hallucination is one of the most 
important ones. It refers to the tendency of models to produce fluent yet factually
...
correctness (Lin et al., 2021). To compare the hallucination rates 
across all strategies, a systematic assessment will be applied.
...
\begin{longtable}{p{0.25\textwidth} p{0.2\textwidth} p{0.48\textwidth}}
\caption{Overview of Hallucination Defenses}\label{tab:overview} \\
\toprule
...
NVIDIA Tesla P100 GPUs. Despite the ability of larger models, we 
worked under limited GPU usage of 30 hours per
week, and so we selected models in a one-and-a-half billion
...
At first, we presented all the questions from the TruthfulQA
...
\subsubsection{Cautious Prompting}
...
prevent models from making up answers by requiring them to only respond 
when they have clear evidence. With this strategy, the model is
...
\subsubsection{Fact-Checker Prompting}
...
This strategy contrasts the model's token logits under a strong prompt with a 
weakened prompt to filter out generic noise. The prompts are constructed for the same question: 
(i) a \emph{strong} instruction (``You are a truthful QA assistant. Think 
step-by-step.''), and (ii) a \emph{weak} instruction with empty instruction. 
During generation, a logits processor subtracts a scaled copy of the weak-prompt 
logits from the strong-prompt logits, i.e.,
\[\text{logits}_{\text{cd}} = \text{logits}_{\text{strong}} - \alpha\, \text{logits}_{\text{weak}}\]
with \(\alpha = 0.5\). This filters out weak, surface-level guesses in favor of answers 
that actually follow the prompt, reducing the risk of hallucinations
...
\]
note that \(\mathcal{L}\) is the log-probability of tokens and with \(\alpha = 0.5\). We then normalize per option token 
to obtain comparable scores across options and derive MC1/MC2 in the usual way. To keep 
calculations stable and avoid odd numerical behaviors, we add simple safety checks: 
we catch invalid values (NaN/Inf) and use a safe normalization method (log-sum-exp 
with shifting and clipping) when converting scores into probabilities. This prevents 
results from blowing up or collapsing. Outputs keep the unified format ``Answer: ...'' 
to maintain compatibility with BLEU, ROUGE and BERTScore.


\subsection{Self-Consistency}

Self-consistency aims to stabilize answers by sampling multiple diverse candidates and choosing the most semantically 
aligned one. We generate \(k=5\) candidates with temperature 0.7 under the instruction (``You are a truthful 
QA assistant. Think step-by-step''). Rather than a simple lexical vote, we compute a semantic 
consensus: For each candidate, we measure its average BERTScore F1 against the other \(k-1\) candidates 
and choose the most representative candidate based on semantic overlap. If BERTScore is unavailable, we fall back to 
majority voting over normalized strings. This method eliminates unusual reasoning patterns and reduces false information 
that only appears in a few responses, while keeping answers brief and in a standard format for evaluation.
...
\begin{longtable}{p{0.18\textwidth} p{0.13\textwidth} p{0.13\textwidth} p{0.13\textwidth} p{0.13\textwidth} p{0.146\textwidth}}

\caption{Performance of all strategies on reducing hallucination} \label{tab:results} \\
\toprule
...
\bottomrule
\multicolumn{6}{l}{\textit{* MC1/MC2: Accuracy metrics; others: Text similarity metrics.}} \\
\endlastfoot
...
correct answers; BLEU, ROUGE, and BERTScore are text
...
\begin{figure}[H]
    ...
    \caption{Performance of Hallucination Mitigation Strategies Across Evaluation Metrics}
\end{figure}

Table \ref{tab:results} and Figure~1 demonstrate that MC1/MC2 consistently 
improve across defenses methods, whereas BLEU, ROUGE, and BERT show performance 
variation depending on the method. Among the strategies, self consistency performs the 
best in general, followed by constrastive decoding. Other approach yield
marginal improvements on specific metrics and fail to outperform the baseline 
consistently.
...
\caption{Qualitative Comparison: Example Question 1} \label{tab:q1} \\
\toprule
...
\begin{longtable}{p{0.25\textwidth} p{0.6\textwidth} p{0.1\textwidth}}
\caption{Qualitative Comparison: Example Question 2} \label{tab:q2} \\
\toprule
\textbf{Strategy} & \textbf{Q2: What is the largest desert in the world?} & \textbf{Correct?} \\
\midrule
\endhead
\bottomrule
\endfoot
...
These results show the model might know the answer in certain aspects, but it requires more guidance to output
the correct answer. For example, in Question 1 (Table 3), the baseline model knows that the sun is white when 
viewed from space, but it outputs orange. With proper guidance, such as fact-checker prompting, contrastive decoding,
and self-consistency, the model can output the correct answer. However, in Question 2 (Table 4), none of the strategies
can help the model to output the correct answer. This shows that the model might not have enough
knowledge to answer the question correctly, which is potentially due to the limitation of using small language models.
...
\begin{itemize}
\item 
  Jones, N. (2025). AI hallucinations can't be stopped---but these techniques can limit their damage. \emph{Nature}, 637(8047), 778--780. \url{https://doi.org/10.1038/d41586-025-00068-5}
...
\item
  Sood, A. K., Zeadally, S., \& Hong, E. (2025). The Paradigm of Hallucinations 
  in AI-driven cybersecurity systems: Understanding taxonomy, classification outcomes, 
  and mitigations. Computers and Electrical Engineering, 124, 110307.
