...
\section*{3 Experiments(to be refined)}
...
These results show the model might know the answer under circumstances, but it needs more guidance to output
the correct answer. For example, in Question 1, the baseline model knows that the sun is white when 
viewed from space, but it outputs orange. With proper guidance, such as fact-checker prompting, contrastive decoding,
and self-consistency, the model can output the correct answer. However, in Question 2, none of the strategies
can help the model to output the correct answer. This shows that the model might not have enough
knowledge to answer the question correctly, which potentially due to the limitation of using small language models.
\section*{4. Conclusion(To be refined)}

Across TruthfulQA, the most reliable mitigation was contrastive decoding, which achieved the highest single-true 
accuracy (MC1=0.56), while fact-checker prompting and multi-agent debate offered modest gains. Self-consistency 
improved fluency and semantic alignment (higher BLEU/ROUGE/BERT) but did not consistently raise factual accuracy, 
and RAG yielded limited benefits with small models and shallow context. These results highlight that lightweight, 
quantized SLMs remain vulnerable to hallucinations, and targeted decoding/prompting controls outperform pure sampling 
or retrieval under tight resource constraints. Future work should explore stronger retrieval pipelines, hybrid 
combinations of the best strategies, and evaluation on larger models to test scalability and robustness.
